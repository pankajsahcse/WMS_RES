...
*/
	...
	$scope.saveEditBudgetRequest = function (isValid) {
		...
		for (var i = 0; i < $('input[name^="workId"]').length; i++) {
			...
			var data = {
				workId: $('input[name^="workId"]')[i].value, requestedAmount: $('input[name^="requestedAmount"]')[i].value,
	...
	$scope.saveSurrenderRequest = function (isValid) {
		...
		for (var i = 0; i < $('input[name^="workId"]').length; i++) {
			...
			var data = {
				workId: $('input[name^="workId"]')[i].value, surrendedAmount: $('input[name^="surrendedAmount"]')[i].value,
	...
	$scope.getLatLngFrommap = function (templat, templong) {
		...
		$http.post('getLatLng/' + templat + '/' + templong + '/')
			.then(function (response) {
				...
				//	console.log("responseData$scope.workData",JSON.stringify($scope.workData));
				...
				//$scope.workData.districtId=$scope.districts[0].districtId+"";
				// alert("$scope.districts[0].districtId ="+$scope.districts[0].districtId)
				if (!$scope.workData.block) {
					$scope.workData.block = {};
				}

				// alert(" $scope.workData.districtId"+$scope.workData.districtId);
				//$scope.loadWdfACByLgdDistCode($scope.workData.districtId);
				...
				*/
				...
				response.success(function (data, status, headers, config) {
					...
					//alert($scope.workData.villageId);
					/*if($scope.workData.gramPanchayatId != null){
				  if (parseInt($scope.gramPanchayat.gramPanchayatId) !== parseInt($scope.workData.gramPanchayatBean.gramPanchayatId )){
					  alert(" you can not Select Out Side Selected GramPanchayat");
							...
							//alert($scope.workData.gramPanchayatId);
				...
				/*$http.post('fetctByGramPanchayatCode/' + $scope.gramPanchayats[0].gramPanchayatId)
					.then(function(response) {
						...
						//alert("$scope.workData.gramPanchayatId: " + $scope.gramPanchayats[0].gramPanchayatId);
					})
	...
	$scope.onchangeDistrict = function () {
		var districtId = $scope.workData.districtId;
	...
	$scope.LoadEstiamtionItemDetails = function () {
		...
		$http.get('fetchItemByChapterForEstimate/' + $routeParams.id)
			.then(function (response) {
				...
				console.error("Error loading estimation item details:", error);
				$loading.finish('sample-1');

			});
	};
	$scope.loadWorkForEMb = function () {
	...
	$scope.openEditMeasurementModal = function (m) {

		// deep copy so table data affect na ho
		$scope.mesurement = angular.copy(m);
		$scope.mesurement.measureLength = Number(m.length);
		$scope.mesurement.measureWidth = Number(m.width);
		$scope.mesurement.measureHeightDepth = Number(m.heightDepth);
		$scope.mesurement.no = Number(m.no);
		$scope.mesurement.currentMesurementNo = Number(m.currentMesurementNo);
		// modal open
		$('#editMeasurementModal').modal('show');
		$scope.loadPreviousDataInEdit(m.estimateSorId, m.measurementId).then(function (data) {
	...
	$scope.loadPreviousData = function (estimateSorId) {

		$loading.start('sample-1');

		return $http.get('loadPreviousDataByEstimateId/' + estimateSorId)
			.then(function (response) {
	...
	$scope.calculateRemainingMeasurement = function () {

		$scope.mesurement.remainingMeasurementL =
			round2(n($scope.mesurement.measureLength) - n($scope.mesurement.previousMeasurementL));

		$scope.mesurement.remainingMeasurementW =
			round2(n($scope.mesurement.measureWidth) - n($scope.mesurement.previousMeasurementW));

		$scope.mesurement.remainingMeasurementT =
			round2(n($scope.mesurement.measureHeightDepth) - n($scope.mesurement.previousMeasurementT_T));


		$scope.mesurement.remainingMeasurementNo =
			round2(n($scope.mesurement.no) - n($scope.mesurement.previousMeasurementNo));
		//alert($scope.mesurement.remainingMeasurementNo +""+$scope.mesurement.no +" =" + $scope.mesurement.previousMeasurementNo )	;
	};

	$scope.loadPreviousDataInEdit = function (estimateSorId, mid) {

		$loading.start('sample-1');

		return $http.get('loadPreviousDataByEstimateIdInEdit/' + estimateSorId + "/" + mid)
			.then(function (response) {
				$scope.previousData = response.data;
	...
	$scope.checkCurrentMeasurementNo = function () {
		...
		if ($scope.mesurement.remainingMeasurementNo < $scope.mesurement.currentMesurementNo) {
			// 🔔 Optional sound


			Swal.fire({
				title: 'Limit Exceeded!',
				html: '<b>Entered measurement exceeds remaining value.</b>',
				imageUrl: 'https://cdn-icons-png.flaticon.com/512/564/564619.png', // 🔺 Red warning triangle
				imageWidth: 90,
				imageHeight: 90,
	...
	$scope.checkCurrentMeasurementL = function () {
		...
		if ($scope.mesurement.remainingMeasurementL < $scope.mesurement.currentMesurementL) {
			///$scope.mesurement.currentMesurementL = 0;
	...
	$scope.checkCurrentMeasurementW = function () {
		if ($scope.mesurement.remainingMeasurementW < $scope.mesurement.currentMesurementW) {
			//$scope.mesurement.currentMesurementW = 0;
			Swal.fire({
				title: 'Limit Exceeded!',
				html: '<b>Entered measurement exceeds remaining value.</b>',
	...
	$scope.checkCurrentMeasurementT = function () {
		if ($scope.mesurement.remainingMeasurementT < $scope.mesurement.currentMesurementT) {
			//	$scope.mesurement.currentMesurementT = 0;
			Swal.fire({
				title: 'Limit Exceeded!',
				html: '<b>Entered measurement exceeds remaining value.</b>',
	...
	$scope.saveMeasurement = function (uploadDoc1, isValid) {

		if (!isValid) {
			return;
		}

		if ($scope.mesurement.currentMesurementW == 0 && $scope.mesurement.currentMesurementL == 0 && $scope.mesurement.currentMesurementT == 0) {
			alert("No Mesurement Remaining");
			return
		}

		if (!confirm("Are you sure you want to save this measurement?")) {
			return;
		}
		...
		// basic fields
		formData.append("measurementId", $scope.mesurement.measurementId || "");
		formData.append("eMbNo", $scope.mesurement.eMbNo);
		formData.append("sorItemNo", $scope.mesurement.sorItemNo);
		formData.append("unit", $scope.mesurement.unit);
		formData.append("itemDesc", $scope.mesurement.itemDesc);
		formData.append("quantity", $scope.mesurement.quantity);

		// measurements
		formData.append("length", $scope.mesurement.measureLength);
		formData.append("width", $scope.mesurement.measureWidth);
		formData.append("heightDepth", $scope.mesurement.measureHeightDepth);
		formData.append("estimateSorId", $scope.mesurement.estimateSorId);
		// previous
		formData.append("previousMeasurementL", Number($scope.mesurement.previousMeasurementL));
		formData.append("previousMeasurementW", Number($scope.mesurement.previousMeasurementW));
		formData.append("previousMeasurementT_T", Number($scope.mesurement.previousMeasurementT_T));
		formData.append("placeDescription", $scope.mesurement.placeDescription);
		formData.append("calculatedAmount", Number($scope.mesurement.calculatedAmount));

		// remaining
		formData.append("remainingMeasurementL", Number($scope.mesurement.remainingMeasurementL));
		formData.append("remainingMeasurementW", Number($scope.mesurement.remainingMeasurementW));
		formData.append("remainingMeasurementT", Number($scope.mesurement.remainingMeasurementT));

		// current
		formData.append("currentMesurementL", Number($scope.mesurement.currentMesurementL));
		formData.append("currentMesurementW", Number($scope.mesurement.currentMesurementW));
		formData.append("currentMesurementT", Number($scope.mesurement.currentMesurementT));
		formData.append("no", $scope.mesurement.no);

		formData.append("currentMesurementNo", Number($scope.mesurement.currentMesurementNo));
		formData.append("remainingMeasurementNo", Number($scope.mesurement.remainingMeasurementNo));
		formData.append("previousMeasurementNo", Number($scope.mesurement.previousMeasurementNo));
		formData.append("remarks", $scope.mesurement.remarks);

		// file
		if (uploadDoc1) {
			formData.append("documentUpload", uploadDoc1);
		}

		$http.post('addMeasurementForThisEmb', formData, {
			transformRequest: angular.identity,
			headers: { 'Content-Type': undefined }
		}).then(function (response) {

			alert(response.data.successMessage || "Saved successfully");
			$('#editMeasurementModal').modal('hide');
			$('#measurementModal').modal('hide');
			$http.get('loadMeasurementByEsorId/' + j.id)
				.then(function (res) {

					$scope.MesurementList = res.data;   // data bind
					// prevent reloading
	...
	$scope.SaveMeasurementWithDsc = function () {
		...
		// Step 1 → get dummy pdf
		$http.post("generateDscVerificationPDF").then(function (res) {

			var base64Pdf = res.data.unsignedBase64PDF;
			var serverTime = res.data.serverDateTime;

			var dscRequest = {
				ServerDateTime: serverTime,
				DocumentType: "PDF",
				Data: base64Pdf,
				SignatureName: "DSC Verification",
				Reason: "Verify before saving",
				Location: "Bhopal"
			};

			return $http.post("http://localhost:8060/jsonsigner/Sign", dscRequest);

		}).then(function (signRes) {

			console.log("DSC RESPONSE:", signRes.data);

			if (!signRes.data || !signRes.data.SignedData) {
				alert("DSC not detected");
				return;
			}

			// 🔥 SHOW ALERT AFTER PASSWORD SUCCESS
			alert("DSC verified successfully");

			// 🔹 Now call save API
			return $http.post("saveMeasurementWithDsc", {
				workId: $routeParams.id,
				signedData: signRes.data.SignedData
			});
			...
			if (saveRes.data.successMessage) {
				Swal.fire({
					icon: 'success',
					text: saveRes.data.successMessage
				});
			} else {
				Swal.fire({
					icon: 'error',
					text: saveRes.data.errorMessage || "Save failed"
				});
			}
	...
	$scope.toggleView = function (j) {

		// 1️⃣ Close all other rows
		angular.forEach($scope.chapterList, function (chapter) {
			angular.forEach(chapter.itemList, function (item) {
				if (item !== j) {
					item.shortView = false;
				}
			});
		});

		// 2️⃣ Toggle current row
		j.shortView = !j.shortView;

		// 3️⃣ Load data only once
		if (j.shortView) {
			...
			$http.get('loadMeasurementByEsorId/' + j.id)
				.then(function (res) {
					...
					// 👈 bind to THIS 		
					j.measurementList = res.data.map(function (m) {
						if (m.valuationDate) {
							m.valuationDate = new Date(m.valuationDate.replace(' ', 'T'));
						}
						return m;
					});
