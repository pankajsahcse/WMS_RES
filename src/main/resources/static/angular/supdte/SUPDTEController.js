...
res.controller('SUPDTEController', function($scope, $loading, $rootScope, $window, $routeParams, $http, $timeout) {
	$scope.started = false;
...
$scope.onVillageChangeDuringEdit = function() {
	...
	$scope.loadWorkRequisitionDetailInSupdte = function() {
	...
	$scope.loadSqmOfficesForSe = function() {

		
		$loading.start('sample-1');
		var response = $http.get('fetchOfficesBySeOfficeIdAndSqmUser/'+$routeParams.id);
		response.success(function(data, status, headers, config) {
			 	...
			 	for(var i=0;i<outer.length;i++){
					...
					responsePromise.success(function(data, status, headers, config) {
						...
						if($rootScope.responseObject.successMessage != null) {
							...
							$window.location.href = '#manageSqmUsersForSe';
						}
						if($rootScope.responseObject.errorMessage != null) {
							$timeout(function() {
								$rootScope.responseObject.errorMessage = null;
